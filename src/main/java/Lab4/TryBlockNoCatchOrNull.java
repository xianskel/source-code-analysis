...
public class TryBlockNoCatchOrNull {
...
//		File file = new File(filePath);
	...
	static class TryStatementVisitor extends VoidVisitorAdapter<Void> {
		public void visit(TryStmt stmt, Void arg) {
			...
			boolean returnsNull = stmt
					.getCatchClauses()
					.stream()
					.anyMatch(
							cBlock -> cBlock.getBody()
							.getStatements()
							.stream()
							.anyMatch(
									statement -> statement.isReturnStmt() 
									&& statement.asReturnStmt().toString().contains("null")));

			if(hasNoCatch || returnsNull) {
				System.out.println(
						"Try Statement with 'no Catch Statement':" + hasNoCatch + " and 'catch returning null':" + returnsNull +
						" in file " + stmt.findAncestor(ClassOrInterfaceDeclaration.class).get().resolve().getClassName()
				);
			}
		}
	}
